...
sdt1 = (sday -29)*24 #for interpolating data
sdt2 = (sday -15)*24 #for csv file
...
metadata = pd.read_csv(homedir+diri2+fil2,sep=',',header=None)
stnnum = metadata[0]
mlat = np.array(metadata[1])
mlon = np.array(metadata[2])

ti1 = "conventional-roof case"
idx = pd.date_range("2018-07-29 00:00:00",periods=dlen,freq='H')


interpT = wi.intp_valid(fil1,mlat,mlon,"T2")[:,sdt1+trunc:sdt1+dlen+trunc] -273.15
interpU = wi.intp_valid(fil1,mlat,mlon,"U10")[:,sdt1+trunc:sdt1+dlen+trunc]
interpV = wi.intp_valid(fil1,mlat,mlon,"V10")[:,sdt1+trunc:sdt1+dlen+trunc]
...
vdata = pd.read_csv(homedir+diri2+fil3,sep=',',header=None)
stndata = vdata[0]
date = vdata[1]
...
#plt.tight_layout()
plt.savefig("../picture/AWS_MEANTW_VALID_PUB.png")
plt.show()
...
#plt.tight_layout()
#plt.show()
plt.savefig("../picture/AWS_MEANWD_VALID_PUB.png")
...
ax[0].set_xlim([datetime.datetime(2018,7,29,0,0,0),datetime.datetime(2018,8,4,0,0,0)])
...
ax[1].set_xlim([datetime.datetime(2018,7,29,0,0,0),datetime.datetime(2018,8,4,0,0,0)])
ax[0].tick_params(which='major',width=1.2,length=7)
ax[0].tick_params(which='minor',width=1.0,length=3)
ax[1].tick_params(which='major',width=1.2,length=7)
ax[1].tick_params(which='minor',width=1.0,length=3)
...
#plt.tight_layout()
plt.savefig("../picture/AWS_VALID_MEAN_SERIES_PUB.png",bbox_inches='tight')
plt.show()
