...
fil3 = 'AWS_0715-0820_forValid3_calculate_revised.csv'
...
interpQ = wi.intp_valid(fil1,mlat,mlon,"Q2")[:,sdt+trunc:sdt+dlen+trunc]
...
odataT, odataws, odatawd, odataq = np.zeros(d2), np.zeros(d2), np.zeros(d2), np.zeros(d2)
mdataT, mdataws, mdatawd, mdataq = np.zeros(d2), np.zeros(d2), np.zeros(d2), np.zeros(d2)
...
for j in range(d2):
	n = 0
	for i in range(d1):
		if not np.isnan(gdata[i,j,7]):
			odataq[j] += gdata[i,j,7]
			mdataq[j] += interpQ[i,j]
			n += 1
	odataq[j] = odataq[j]/float(n)
	mdataq[j] = mdataq[j]/float(n)
...
ME = 0.
for i in range(dlen):
    if pd.notnull(odataq[i]):
        diff = mdataq[i] - odataq[i]
        VAR += diff**2
        ME += diff
        n1 += 1
        if diff<=1. and diff>=-1:
...
#print("Hit rate of q = {}".format(HRT))
print("RMSE of q = {}".format(RMSE))
print("Mean error of q = {}".format(ME))
...
ax.set_xlabel('observed mixing ratio (g/kg)',fontsize=fs)
ax.set_ylabel('simulated 2-m mixing ratio (g/kg)',fontsize=fs)
ax.set_xlim(sx,ex)
ax.set_ylim(sx,ex)
ax.scatter(odataq,mdataq,s=1,color='k',alpha=a)
...
#plt.tight_layout()
#plt.savefig("../picture/aws_meanq_valid_pub.png")
#plt.show()
"""
...
ax[2].set_xlabel('observed mixing ratio (g kg$^{-1}$)',fontsize=fs)
ax[2].set_ylabel('simulated 2-m mixing ratio (g kg$^{-1}$)',fontsize=fs)
ax[2].set_xlim(sx,ex)
ax[2].set_ylim(sx,ex)
ax[2].scatter(odataq,mdataq,s=1,color='k',alpha=a)
ax[2].tick_params(axis = 'both',which='both',labelsize=ls)
#ax.xticks(np.arange(sx,ex+1,1))
#plt.yticks(np.arange(sx,ex+1,1))
ax[2].plot([sx,ex],[sx,ex],'k')
ax[2].tick_params(which='major',width=1.2,length=7)
ax[2].tick_params(which='minor',width=1.0,length=3)
...
ax[2].set_aspect(1)
plt.subplots_adjust(left=0.15,bottom=0.15,top=0.90,wspace=0.40)
fig.text(0.15,0.894,'(a)',fontsize=fsp1+5)
fig.text(0.426,0.894,'(b)',fontsize=fsp1+5)
fig.text(0.702,0.894,'(c)',fontsize=fsp1+5)



#plt.tight_layout()
plt.savefig("../picture/aws_meantwq_valid_"+fil1+"_hor.png")
plt.show()
