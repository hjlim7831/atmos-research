...
for i in range(tday):
	maxuhi[i] = max(uhi[24*i:24*(i+1)])
	maxw[i] = max(muw1[24*i:24*(i+1)])
	for j in range(24):
...
plt.plot(xx,yyr,'k')
plt.xlabel('Maximum 10 m Wind Speed ($^\circ$C)')
plt.ylabel('Maximum White roof effect ($^\circ$C)')
plt.savefig('./scatter_ws_max_UHI_compare.png')
#plt.show()
