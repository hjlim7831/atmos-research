...
varbox[6,:] = np.nanmean(HWmeT_2018,axis=1)
varbox[7,:] = np.nanmean(HWmeT,axis=1)
varbox[8,:] = varbox[6,:] - varbox[7,:]
varbox[9,:] = np.nanmean(HWmiT_2018,axis=1)
varbox[10,:] = np.nanmean(HWmiT,axis=1)
varbox[11,:] = varbox[9,:] - varbox[10,:]
varbox[12,:] = np.nanmean(HWmer_2018,axis=1)
varbox[13,:] = np.nanmean(HWmer,axis=1)
varbox[14,:] = varbox[12,:] - varbox[13,:]
varbox[15,:] = np.mean(HWmir_2018,axis=1)
varbox[16,:] = np.nanmean(HWmir,axis=1)
varbox[17,:] = varbox[15,:] - varbox[16,:]


f = open('varboxv4.csv','w',newline='')
wr = csv.writer(f)
wr.writerow(var)
for i in range(nstn):
