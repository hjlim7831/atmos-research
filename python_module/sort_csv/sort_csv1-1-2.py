...
class sep_csv:
	def __init__(self,stndata,rdata,date): #rdata = ndarray
		...
		if dim == 1:
			...
			self.vsize = vsize
			mdata = np.full((dim,dsize,vsize),-999.9999)  #dim: number of stations, dsize: largest size of each station data
...
#for yyyy-mm-dd
	...
	def yymmdd(self,syr,smt,sd,eyr,emt,ed,format="yy-mm-dd"): 
		s = format
		ss = s.replace("yy","{0:04d}")
		ss = ss.replace("mm","{1:02d}")
		ss = ss.replace("dd","{2:02d}")
		if syr != eyr:      #I'll make this later...
			i1 = smt -1
			i2 = emt -1
			md1 = mdays(syr)
			md2 = mdays(eyr)
			...
			self.DATE = DATE
			self.tlen = tlen
#for yyyy-mm-dd hh:00
	def yymmddhh(self,syr,smt,sd,eyr,emt,ed,format="yy-mm-dd hh:00",zero=0):
		s = format
		ss = s.replace("yy","{0:04d}")
		ss = ss.replace("mm","{1:02d}")
		ss = ss.replace("dd","{2:02d}")
		...
		if syr != eyr:
			...
			self.DATE = DATE
			self.tlen = tlen
	def date_csv(self,mdata,rdate,DATE):
		tlen = self.tlen
		dsize = self.dsize
		dim = self.dim
		DATE = self.DATE
		...
		if tlen >= dsize:
			if dim == 1:
				gdata = np.full((tlen,vsize),-999.9999)
				n = 0
				for j in range(tlen):
					if n <dsize:
						print(DATE[j])
				...
				same = self.same
				mis = tlen - same
				gdata = np.full((dim,tlen,vsize),-999.9999)
				for i in range(dim):
					n = 0
					for j in range(tlen):
						if n < dsize:
